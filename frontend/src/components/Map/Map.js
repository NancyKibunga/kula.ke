...
export default function Map({ readonly, location, onChange }) {
  return (
    <div className={classes.container}>
      ...
      >
        {/* responsible for showing tile images of the map */}
...
// for finding my location button using the value assigned from the checkout page
function FindButtonAndMarker({ readonly, location, onChange }) {
  const [position, setPosition] = useState(location);
// listening to the position changes
  useEffect(() => {
    if (readonly) {
      map.setView(position, 13);
      return;
    }
    // if position is not null or undefined, we call the on change
